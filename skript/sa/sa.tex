...
\end{figure}\\
Dieser Vorgang wird nun eine bestimmte Anzahl mal oder solange bis keine "Anderung mehr vorliegt durchgespielt.\\
Dabei ist klar, dass die Qualit"at der so gefunden L"osung sowohl von der Startkombination $\vec{x}_0$, als auch von der Bestimmung der Nachbarschaft $V(\vec{x})$ abh"angt. Gleichzeitig ist die Gefahr gross, in einem lokalen Minimum der Zielfunktion h"angen zu bleiben (siehe Abb. \ref{localsearch}).
\begin{figure}[ht!]
\usetikzlibrary{decorations.pathreplacing} 
\usetikzlibrary{calc}
\centering
