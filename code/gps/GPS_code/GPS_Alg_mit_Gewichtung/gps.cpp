...
void matGausAlg(double* M, double* xi, double* yi, double* zi, double* ti, double* p, int n, double* ci);
void vecBGausAlg(double* M, double* xi, double* yi, double* zi, double* ti, double* p, int n, double* ci);
void matHess(double* M, double* xi, double* yi, double* zi, double* ti, double* p, int n, double* ci);
...
int main(void)
{
  ...
  double* c = new double [anzSat];

  for(int i = 0; i < anzSat; i++)
  {
    //cout << "Satellit " << i+1 << ": x, y, und z-Koordinate, Zeit sowie Standardabweichung eingeben" << endl;
    cin >> x[i] >> y[i] >> z[i] >> t[i] >> c[i];
  }

  // Satellitendaten zur Kontrolle ausgeben:
  cout << endl;
  cout << "Satellitendaten:" << endl;
  cout << "                 x-Position:    y-Position:    z-Position:          Zeit:   Standardab.: " << endl;
  for(int i = 0; i < anzSat; i++)
  {
  ...
  //cout << "Zielfunktion im optimalen Punkt: " << zf(x,y,z,t,p0,anzSat,c) << endl << endl;
  ...
  {  
    ...
    {  
      ...
      { 
        ...
        matGausAlg(G,x,y,z,t,pn,anzSat,c);
        vecBGausAlg(b,x,y,z,t,pn,anzSat,c);
  ...
  if((radius>(erdradius-radTol))&&(radius<(erdradius+radTol))&&(exTyp==1))
  {
    ...
    cout << "Zielfunktion: " << zf(x,y,z,t,pn,anzSat,c) << endl;
...
void matGausAlg(double* M, double* xi, double* yi, double* zi, double* ti, double* p, int n, double* ci)
{
  M[0] = fxx(xi,yi,zi,ti,p,n,ci);
  M[1] = fxy(xi,yi,zi,ti,p,n,ci);
  M[2] = fxz(xi,yi,zi,ti,p,n,ci);
  M[3] = fxt(xi,yi,zi,ti,p,n,ci);

  M[4] = M[1];
  M[5] = fyy(xi,yi,zi,ti,p,n,ci);
  M[6] = fyz(xi,yi,zi,ti,p,n,ci);
  M[7] = fyt(xi,yi,zi,ti,p,n,ci);

  M[8] = M[2];
  M[9] = M[6];
  M[10] = fzz(xi,yi,zi,ti,p,n,ci);
  M[11] = fzt(xi,yi,zi,ti,p,n,ci);

  M[12] = M[3];
  M[13] = M[7];
  M[14] = M[11];
  M[15] = ftt(xi,yi,zi,ti,p,n,ci);
}

void vecBGausAlg(double* M, double* xi, double* yi, double* zi, double* ti, double* p, int n, double* ci)
{
  M[0] = -fx(xi,yi,zi,ti,p,n,ci);
  M[1] = -fy(xi,yi,zi,ti,p,n,ci);
  M[2] = -fz(xi,yi,zi,ti,p,n,ci);
  M[3] = -ft(xi,yi,zi,ti,p,n,ci);
}
