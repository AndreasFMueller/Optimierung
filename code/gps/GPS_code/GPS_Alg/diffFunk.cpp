...
double zf(const double* xi, const double* yi, const double* zi, const double* ti, const double* p, const int n)
{
  ...
  for(j=0; j<n; j++)
  {
    f = f + ((xi[j]-p[0])*(xi[j]-p[0]) + (yi[j]-p[1])*(yi[j]-p[1]) + (zi[j]-p[2])*(zi[j]-p[2]) - c2*(p[3]-ti[j])*(p[3]-ti[j])) 
...
double fx(const double* xi, const double* yi, const double* zi, const double* ti, const double* p, const int n)
{
  ...
  for(j=0; j<n; j++)
  {
    f = f + (xi[j]-p[0]) * ((xi[j]-p[0])*(xi[j]-p[0]) + (yi[j]-p[1])*(yi[j]-p[1]) + (zi[j]-p[2])*(zi[j]-p[2]) - c2*(p[3]-ti[j])*(p[3]-ti[j]));
  }
  return -4*f;
}
...
double fy(const double* xi, const double* yi, const double* zi, const double* ti, const double* p, const int n)
{
  ...
  for(j=0; j<n; j++)
  {
    f = f + (yi[j]-p[1]) * ((xi[j]-p[0])*(xi[j]-p[0]) + (yi[j]-p[1])*(yi[j]-p[1]) + (zi[j]-p[2])*(zi[j]-p[2]) - c2*(p[3]-ti[j])*(p[3]-ti[j]));
  }
  return -4*f;
}
...
double fz(const double* xi, const double* yi, const double* zi, const double* ti, const double* p, const int n)
{
  ...
  for(j=0; j<n; j++)
  {
    f = f + (zi[j]-p[2]) * ((xi[j]-p[0])*(xi[j]-p[0]) + (yi[j]-p[1])*(yi[j]-p[1]) + (zi[j]-p[2])*(zi[j]-p[2]) - c2*(p[3]-ti[j])*(p[3]-ti[j]));
  }
  return -4*f;
}
...
double ft(const double* xi, const double* yi, const double* zi, const double* ti, const double* p, const int n)
{
  ...
  for(j=0; j<n; j++)
  {
    f = f + (p[3]-ti[j]) * ((xi[j]-p[0])*(xi[j]-p[0]) + (yi[j]-p[1])*(yi[j]-p[1]) + (zi[j]-p[2])*(zi[j]-p[2]) - c2*(p[3]-ti[j])*(p[3]-ti[j]));
  }
  return -4*c2*f;
}
...
double fxx(const double* xi, const double* yi, const double* zi, const double* ti, const double* p, const int n)
{
  ...
  for(j=0; j<n; j++)
  {
    f = f + 3*(xi[j]-p[0])*(xi[j]-p[0]) + (yi[j]-p[1])*(yi[j]-p[1]) + (zi[j]-p[2])*(zi[j]-p[2]) - c2*(p[3]-ti[j])*(p[3]-ti[j]);
  }
  return 4*f;
}
...
double fyy(const double* xi, const double* yi, const double* zi, const double* ti, const double* p, const int n)
{
  ...
  for(j=0; j<n; j++)
  {
    f = f + (xi[j]-p[0])*(xi[j]-p[0]) + 3*(yi[j]-p[1])*(yi[j]-p[1]) + (zi[j]-p[2])*(zi[j]-p[2]) - c2*(p[3]-ti[j])*(p[3]-ti[j]);
  }
  return 4*f;
}
...
double fzz(const double* xi, const double* yi, const double* zi, const double* ti, const double* p, const int n)
{
  ...
  for(j=0; j<n; j++)
  {
    f = f + (xi[j]-p[0])*(xi[j]-p[0]) + (yi[j]-p[1])*(yi[j]-p[1]) + 3*(zi[j]-p[2])*(zi[j]-p[2]) - c2*(p[3]-ti[j])*(p[3]-ti[j]);
  }
  return 4*f;
}
...
double ftt(const double* xi, const double* yi, const double* zi, const double* ti, const double* p, const int n)
{
  ...
  for(j=0; j<n; j++)
  {
    f = f + (xi[j]-p[0])*(xi[j]-p[0]) + (yi[j]-p[1])*(yi[j]-p[1]) + (zi[j]-p[2])*(zi[j]-p[2]) - 3*c2*(p[3]-ti[j])*(p[3]-ti[j]);
  }
  return -4*c2*f;
}
