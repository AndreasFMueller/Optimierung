/*
 * simplexlapack.c -- LAPACK backend for the Simplex Algorithm
 *
 * (c) 2013 Prof Dr Andreas Mueller, Hochschule Rapperswil
 */
#include <simplexlapack.h>
#include <backend.h>
#include <stdio.h>
#include <stdlib.h>
#include <globals.h>

int simplexlapack_version = 1;

static int	simplexlapack_pivot(backend_t *backend,
...
static backend_t	simplexlapack_backend = {
	.private_data = NULL,
	.init = NULL,
	.release = NULL,
	.pivot = simplexlapack_pivot
};

static void	simplexlapack_register() __attribute__ ((constructor));
static void	simplexlapack_register() {
	if (backend_register("LAPACK", &simplexlapack_backend) < 0) {
		fprintf(stderr, "cannot register LAPACK backend\n");
		exit(EXIT_FAILURE);
	}
	fprintf(stderr, "LAPACK backend registered\n");
}
